<?php 
    ...
    $obj = new StudyMeet();
    $result=$obj->get_manage_language();
?>
      ...
      <div class="col-md-12 col-sm-12 col-xs-12">
         <div class="x_panel">
            ...
            <ul class="nav navbar-right panel_toolbox">
              <li>
                <a href="<?php echo 'add_language.php';?>">
                        ...
                        ?>
                            <tr>
                                ...
                                <td><?php if ($data->status=="1") { ?>
                                        <a href="javascript:void(0);" class="btn btn-primary inactive_language" id="<?php echo base64_encode($data->language_id); ?>">Active</a>
                                    <?php }else if ($data->status=="2") { ?>
                                        <a href="javascript:void(0);" class="btn btn-danger active_language" id="<?php echo base64_encode($data->language_id); ?>">InActive</a>
                                    <?php } ?></td>
                                <td> 
                                    <a href="<?php echo ADMIN_URL.'add_language.php?id='.base64_encode($data->language_id); ?>" class="btn btn-primary">Edit</a>
                                    <a href="javascript:void(0);" class="btn btn-danger remove_code" id="<?php echo base64_encode($data->language_id); ?>">Remove</a>
                                </td>
                            </tr>
                    <?php } } ?>
                  </tbody>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(document).on('click','.remove_code',function(){
            ...
            if(confirm("Are you sure you want to Remove Language?")){
                $.ajax({
                    ...
                    data: {remove_language_id:language_id},
        ...
        $(document).on('click','.active_language',function(){
            var language_id = $(this).attr("id");
            if(confirm("Are you sure you want to Activated This Language?")){
                change_satus(language_id,"1");
            }
        });
        $(document).on('click','.inactive_language',function(){
            var language_id = $(this).attr("id");
            if(confirm("Are you sure you want to InActive This Language?")){
                change_satus(language_id,"2");
            }
        });
    });
    function change_satus(language_id,status){
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo AJAX_URL;?>",
            data: {change_status_language_id:language_id,status:status},
            success: function(data){                            
                if(data.success){
                    $('.success-msg').removeClass("hidden");
                    $('.success-msg').html(data.message);
