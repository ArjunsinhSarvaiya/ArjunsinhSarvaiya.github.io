<?php 
    ...
    $obj = new StudyMeet();
    $result=$obj->getmst_mst_campaign(array('user_id' => 1,'user_type' => 1, ));
   ...
   <div class="page-title">
        ...
        <div class="x_title">
            <ul class="nav navbar-right panel_toolbox">
              <li>
                <a href="<?php echo 'add_campings.php';?>">
                  ...
                  <tbody>
                    ...
                    <?php foreach ($result as $results) {
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="<?php echo ADMIN_URL.'add_campings.php?id='.base64_encode($results->campaign_id); ?>" class="btn btn-primary">Edit</a>
                                ...
                                <a href="<?php echo ADMIN_URL.'Campaign_Ranking.php?id='.base64_encode($results->campaign_id).'&by=admin'; ?>" class="btn btn-primary">See All Rank</a>
                                <?php if($results->result=="0" && $results->status!="1"){ ?>
                                    <a href="javascript:void(0);" class="btn btn-primary release_result" id="<?php echo $results->campaign_id; ?>">Release Result</a>
                                <?php } ?>
                            </td>
                        </tr>
                   <?php } } ?>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(document).on('click','.release_result',function(){
            var campaign_id = $(this).attr('id');

            if(confirm("Are you sure you want to Release Result?")){
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "<?php echo AJAX_URL;?>",
                    data: {release_campaign_id:campaign_id},
                    success: function(data) {                            
                        if(data.success){
                            $('.success-msg').removeClass("hidden");
                            $('.success-msg').html(data.message);
